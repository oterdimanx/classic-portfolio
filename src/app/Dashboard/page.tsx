...
import { setUserLoading, setUserData, setCatLoading, setCategoryData, setOrderData, setProdLoading, setProductData, setLookbookLoading, setLookbookData, setArchiveLoading, setArchiveData } from '@/utils/AdminSlice';
...
export default function Dashboard() {
  ...
  const { data: userData, isLoading: userLoading } = useSWR('/gettingAllUsersFOrAdmin', get_all_users)
  ...
  }, [categoryData, dispatch, categoryLoading, productData, productLoading, orderData, orderLoading, lookbookData, lookbookLoading, archiveData, archiveLoading, userData, userLoading ])
    ...
    <div className="w-full h-screen flex bg-gray-50 overflow-hidden font-[Poppin]">
      ...
      <div className="w-full h-full font-[Poppin]">
        ...
        <div className="w-full h-5/6 flex flex-wrap items-start justify-center overflow-y-auto font-[Poppin] px-4 py-2">
          {categoryLoading || productLoading || lookbookLoading || archiveLoading || userLoading ? <Loading /> : <SuperComponent />}
        </div>
      </div>
    </div>
  )
