...
export async function POST(req: Request) {
  try {
    ...
    if (isAuthenticated === 'admin') {
      ...
      console.log("Incoming data from form:", data);
      
      // Map frontend field names to match your schema
      const mappedData = {
      ...
      console.log("Mapped data for validation and DB:", mappedData);
      
      // Validate with your schema
      const { error } = AddProductSchema.validate(mappedData);
      if (error) {
      ...
      // Insert into MongoDB
      const saveData = await Product.create(mappedData);
