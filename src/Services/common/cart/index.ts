...
export const update_cart_items = async (formData: any) => {
  try {
    const res = await fetch(`/api/common/cart/update-cart-items`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${Cookies.get('token')}`
    ...
    console.log('Error in update cart items (service) =>', error)
  }
}
