...
export default function CartCard({ productID, userID, _id , quantity, isOverlay = false }: Data) {
    ...
    const updateCartData = async (newCart: Data[] | undefined) => {
        const newUpdateCart = newCart?.map((item: Data) => {
            if (item?._id === _id) {
                if (item?.productID?.productQuantity > item?.quantity) {
                    const updatedCartData = update_cart_items(item)
                }
            }
            return item
        })
    ...
    const handleIncrement = () => {
        ...
        if (qnt > 0) {
            console.log('update basket 1')
            let quantity = qnt + 1
            setQnt(quantity)
            dispatch(setCart(newCart))
        }
        else {
            console.log('update basket 2')
            setQnt(quantity)
            dispatch(setCart(newCart))
        }
    }
    ...
    return ( 
        /** 
         * insert this in the first className in case basket needs to be styled
         * differently in overlay : ${isOverlay ? 'bg-white' : 'bg-black'}
         * replace bg-values with your choices
         */
        <div className={`w-full bg-white rounded-xl m-2 border-b flex-col md:flex-row h-72  md:h-40 py-2 px-4 flex justify-around items-center`}>
            <Image 
                src={productID?.productImage || '/pants.png'} 
                alt="no image found" 
                width={100} 
                height={150} 
                className="rounded" />
            ...
            <div className="flex  justify-center items-center">
                <button 
                    ...
                    className="btn btn-circle dark:text-white  text-xl">-</button>
